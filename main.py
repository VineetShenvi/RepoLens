...
from db.neo4j import store_graph
...
async def analyze(session_id: str, repo_input: RepoInput):
    ...
    if sha and entry.get('sha') == sha and entry.get('report'):
        ...
        session_repo_store[session_id] = {'cache_key': cache_key, 'sha': sha}
    ...
    async def stream_and_store():
        # Clear raw-chunk store so previous repo's data doesn't bleed into KG creation
        try:
            client.delete_collection(collection_name="documents")
            logger.info("[%s] /analyze — cleared 'documents' collection for fresh ingest", session_id)
        except Exception:
            pass
...
async def websocket_chat(websocket: WebSocket, session_id: str):
    ...
    try:
        ...
        # Check if this repo+SHA was already fully indexed
        ...
        if sha and cache_key:
            ...
            active = ws_cache.get('_active', {})
            is_active_repo = active.get('cache_key') == cache_key and active.get('sha') == sha
            if ws_entry.get('sha') == sha and ws_entry.get('indexed') and is_active_repo:
                try:
                    client.get_collection("graph_documents")
                    logger.info("[%s] Cache hit — skipping KG pipeline for %s @ %s", session_id, cache_key, sha[:8])
                    await websocket.send_text("__LOG__:Using cached knowledge graph...")
                    await websocket.send_text("__READY__")
                    await run_chat_loop(websocket, session_id)
                    return
                except Exception:
                    logger.info("[%s] Qdrant collection missing despite cache — re-indexing", session_id)

        t0 = time.perf_counter()
        logger.info("[%s] Starting create_knowledge_graph...", session_id)
        list_graph_docs = await create_knowledge_graph()
        logger.info("[%s] create_knowledge_graph done — %s graph docs in %.1fs", session_id, len(list_graph_docs), time.perf_counter() - t0)

        try:
            client.delete_collection(collection_name="graph_documents")
            logger.info("[%s] Deleted existing Qdrant 'graph_documents' collection", session_id)
        except Exception:
            pass

        if list_graph_docs:
            await websocket.send_text("__LOG__:Clearing Neo4j graph...")
            logger.info("[%s] Clearing Neo4j graph...", session_id)
            graph.query("MATCH (n) DETACH DELETE n")
            logger.info("[%s] Neo4j cleared", session_id)
            ...
            store_knowledge_graph(list_graph_docs)
            logger.info("[%s] store_knowledge_graph done in %.1fs", session_id, time.perf_counter() - t2)

            # Mark as indexed and set as the active repo in Qdrant/Neo4j
            if sha and cache_key:
                ...
                if updated_entry.get('sha') == sha:
                    ...
                    updated_cache['_active'] = {'cache_key': cache_key, 'sha': sha}
                    _save_cache(updated_cache)
                    logger.info("[%s] Marked %s as indexed and active in cache", session_id, cache_key)
        else:
